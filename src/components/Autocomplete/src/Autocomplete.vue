...
export default {
	...
	methods: {
		...
		validateComponentOptions() {
			assert.warn(!this.hasSearchIcon, 'Autocomplete', 'The `has-search-icon` prop has been deprecated and will be removed in the next major release of Orbit. Please use the `prefix` slot instead.');
			...
			if (this.multiSelect && (this.prefix || this.$slots.prefix)) {
				throwError('Autocomplete', 'The `prefix` can only be used with single-select Autocompletes.');
			}
