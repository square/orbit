...
import Key from './Key';
import { CLICKABLE_ALL, CLICKABLE_NONE } from './Constants';
...
export default {
	...
	computed: {
		...
		isClickable() {
			...
			// Handle all three cases: all, completed, none
			if (this.parent.clickable === CLICKABLE_NONE) {
				return isClickable;
			}

			if (this.status !== 'active') {
				if (this.parent.clickable === CLICKABLE_ALL) {
					isClickable = true;
				} else {
